 ...
 if ($toal_mathang > 0) {
   ...
   while ($rows_mathang = $dbf->nextdata($rst_mathang)) {
     ...
     <tr class="titleBottom">
        ...
        if ($mathang_id != '')
         {
             $rs_nhapkho = $dbf->getDynamic("nhapkho2", "datecreated >= " . $tungay . " and datecreated <= " . $denngay . " and mathang_id='" . $id_mathang . "' and status=1", "datecreated asc");
         }
         else {
           $rs_nhapkho = $dbf->getDynamic("nhapkho2", "datecreated >= " . $tungay . " and datecreated <= " . $denngay . " and status=1", "datecreated asc");
         }
        ...
        if ($total_nhapkho > 0) {
          ...
          while ($row_nhapkho = $dbf->nextData($rs_nhapkho)) {
                ...
                if($ngay_tem=="")
                {
                   ...
                   echo '<tr class="cell1">
                            <td colspan="7"><h4>Ngày nhập: '.$datecreated.'</h5></td>
                          </tr>';
                }else
                {
    ...
    <tr class="<?php echo (($i % 2 == 0) ? "cell2" : "cell1") ?>">
      ...
      ?>
       ...
       <tr class="cell2">
          <td colspan="6" style="text-align: right"> Tổng số:</td>
         ...
         <?php
            ...
            echo "<tr class='cell2'><td colspan='7'><h3 style='text-align:left'> =>Không có dữ liệu</h3></td></tr>";
          }
          ?>
