...
-----------------------------------------------------------------*/
  ...
  function getInfoColum_code($table, $item_code) {
    $result = $this->getDynamic($table, "item_code='" . $item_code . "'", "");
    if ($this->totalRows($result) > 0) {
      $row = $this->nextData($result);
      return $row;
    }
...
-----------------------------------------------------------------*/
  function updateTable($tbName, $arrayValue, $condition, $type = "UPDATE") {
    try {
      ...
      if ($type == "INSERT")
        $sql = "INSERT INTO " . $tbName . " SET " . $str;
      else
  ...
  function getMetaData($tableName, $type = '') {
    try {
      ...
      while ($field = $this->nextObject($result, 0)) {
        ...
        if (strstr($fType, "int") != '')
          $fType = "int";
        elseif ($fType == "double" || $fType == "float" || $fType == "decimal")
          $fType = "real";
...
//length field
      $array [] = $array4;
//extension(primary key, tablename)
      $array [] = $array5;
//extension(primary key, tablename)
...
//get comment of field
  ...
  function getMetaData_Mannager($tableName) {
    $result = $this->doSQL("show full fields from " . $tableName);
    $flag = false;
    $i = 1;
    $list_orderby = array();
    while ($field1 = $this->nextObject($result, 0)) {
      ...
      if (strstr($field['Type'], "int") != '')
        $field['Type'] = "int";
      elseif ($field['Type'] == "double" || $field['Type'] == "float" || $field['Type'] == "decimal")
        $field['Type'] = "real";
...
//$this->doSQL("DELETE FROM sys_field WHERE (table_id=".$table_id.") AND (field_name not in (".implode(',',$arrayField)."))");
  ...
  function processLogin($username, $password) {
	...
	if ($this->totalRows($result) > 0) {
      ...
      $_SESSION["user_login"]["group_id"] = $row["group_id"];
      ...
      $_SESSION["user_login"]["sales_code"] = $row["sales_code"];
      $_SESSION["user_login"]["sales_agencies_coms_id"] = $row["sales_agencies_coms_id"];
