...
<div class="panelView" id="panelView">
    ...
    <center>
            <H1>CHI TIẾT BÁN HÀNG: CÔNG NỢ</H1>
            <p>Tất Cả CÔNG NỢ</p>
    </center>
...
<tbody>
    ...
    <?php
        ...
        $rs_hoadon=$dbf->getDynamic("hoadon","(thanhtoan_id=6 or thanhtoan_id=8) and status=1","khachang_id,datecreated asc ");
        ...
        if($total_hoadon>0)
        {
            ...
            while($row_hoadon=$dbf->nextData($rs_hoadon)){
              ...
              if($thanhtoan_id==8){
                 $tongsotienthongke+= $tongsotien - $datcoc;
                 $tongtieno=$tongsotien - $datcoc;
              }else
              {
                 $tongsotienthongke+= $tongsotien;
                 $tongtieno= $tongsotien;
              }
              $khachhang_id = stripcslashes($row_hoadon['khachang_id']);

              if($id_khachang_term==0)
              {
               $id_khachang_term = $khachhang_id;
              }
             ...
             if($id_khachang_term==$khachhang_id)
              {
                  $tongtien_group+= $tongtieno;
              ...
              }else
              {
               ?>
                   <tr class="cell2">
                    <td colspan="7" style="text-align: right"> Tổng số tiền nợ:</td>
                    <td colspan="2" style="text-align: left; color: red;font-size:14px;"><b><?php echo $utl->format($tongtien_group)?> <sup>đ</sup></b></td>
                   </tr>
                   <tr class="cell2"><td colspan="9" style="text-align: right">&nbsp;</td> </tr>
                <?php
                   $tongtien_group = 0;
                   $id_khachang_term = $khachhang_id;
                   $tongtien_group+= $tongtieno;   
        ...
        <td colspan="9" style="background: none">
             <table class="table_dichvu">
              ...
              ?>
                  ...
                  ?>
                  <tr>
                      <td colspan="5" style="text-align: right">Thành tiền nợ</td><td align="center"><b><?=$utl->format($tongtieno)?></b></td>
                  </tr>
             </table>
        </td>
    </tr>
